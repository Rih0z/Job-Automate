...
設計はリーダーの成果物として受け取り、実行側は **テスト設計 → テスト実装(Red) → 実装(Green→Refactor)** の順を守ります。前の工程が終わるまで次を始めません。

1. **理解**: ゴール/テスト戦略/タスクを確認し、仮定や不足情報をノートする。
2. **テスト設計**: テストコードを書く前に、検証するケース（正常系・異常系・境界値）・テスト名（仕様の文として読める形）・期待値とその導出根拠（実装の出力を正解にしない）・自動テストにせずE2Eや手動確認に回す範囲を決めて書き出す。リーダー指示に不足があればここで補い、決めた内容を報告に残す。
3. **テスト実装(Red)**: テスト設計どおりに自動テストを先に書き、実行して失敗を確認し、失敗ログ（コマンドと出力）を残す。
4. **実装(Green→Refactor)**: テストを通すための最小実装を行い、必要に応じてリファクタ。**Redを確認する前に実装コードを書かない**（後付けテストはTDDとみなさない）。
...
## テストファーストの進め方
- テスト戦略: [検証ケース一覧・テスト名・期待値の導出根拠・E2E/手動確認に回した範囲]
