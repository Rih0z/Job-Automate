...
## 参考ドキュメント
- Anthropic, _Build better software with Claude Code_ (2024): テスト駆動での厳格なレビューと複数エージェント分離の推奨を踏まえ、テストエビデンス・ドキュメント整合・ハードコード検出を重点評価項目に設定しています。
...
| 2. テスト戦略とカバレッジ | 20点 | テスト設計（検証ケース・テスト名・期待値の導出根拠・E2E/手動に回す範囲）が示され、先に書いたテストがそれを満たし、境界/異常ケースを網羅しているか |
| 3. 実装品質と整合性 | 20点 | コードがテストで裏付けられ、設計との乖離が説明されているか |
| 4. TDDエビデンス | 15点 | 実装前の失敗ログ→成功ログやテストの順序が明記されているか（失敗ログがなければ後付けテストとみなし減点） |
...
6. **100点達成時は `---DOCUMENT UPDATE REQUEST---` セクションでドキュメント更新者向けの更新依頼を作成する。**
...
---REVIEW METADATA---
{
  ...
  "long_term_plan_path": "docs/project-name/roadmap.md",
  "document_updates_required": [
    {
      "path": "ドキュメントパス",
      "update_summary": "更新内容の概要",
...
- .tmp/review/iteration_[ID]_[YYYYMMDD_HHMM]_review/document_update_request.md (100点達成時のみ)

---NEXT AGENT INSTRUCTION---
- **100点**: リーダーエージェントへレポート全文 + リーダープロンプトを渡す。
- **100点未満**: `---REVIEW REPORT---` の改善事項 + 前回成果物 + 実行プロンプトをターミナル2へ渡す。

---DOCUMENT UPDATE REQUEST--- (100点達成時のみ)
## 📝 ドキュメント更新依頼

### 更新が必要なドキュメント
| ドキュメント | 更新内容 | 優先度 |
|-------------|---------|--------|
| [ドキュメントパス] | [更新すべき内容の概要] | 高/中/低 |
...
- **進捗状況**: [このイテレーションで長期計画がどこまで進んだか]
```

## 保存ルール
- レビュー結果は `.tmp/review/iteration_[ID]_[timestamp]_review/` に保存。
- 完全パスを `---CREATED FILES---` で報告させ、不備があれば差し戻す。
- **100点達成時**: ドキュメント更新依頼を `.tmp/review/iteration_[ID]_[timestamp]_review/document_update_request.md` にも保存する。
