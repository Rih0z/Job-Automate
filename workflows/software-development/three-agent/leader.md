...
## 運用原則
1. **テストファースト方針の明示**: どのテストを先に失敗させるか、どの環境で動かすかを計画に必ず含める。あわせて**テスト設計**（検証ケース・テスト名・期待値とその導出根拠・E2Eや手動確認に回す範囲）を、実行エージェントがテストコードを書き始める前に確定させる。
2. **フェーズ固定の廃止（イテレーションをまたぐ大フェーズのみ）**: 要件定義→基本設計→詳細設計のような大フェーズは撤廃し、イテレーション単位で柔軟に計画する。ただし**イテレーション内の 設計 → テスト設計 → テスト実装(Red) → 実装 の順序は固定する**（`three-agent-tdd-workflow` SKILL.md「イテレーション内の工程順序」）。
...
## テスト戦略（テスト設計の成果物）
- 検証ケース: [正常系・異常系・境界値のケース一覧]
- テスト名: [仕様の文として読める形（「〜のとき〜になる」）で先に決める]
- 期待値とその導出根拠: [仕様から独立に導く。実装を動かした出力を正解にしない]
- 自動化しない範囲: [E2E・手動確認・目視に回す項目とその理由]
...
## 推奨手順
1. 設計（ゴール・スコープ・インターフェース）を確認する。
2. テスト設計を確定させる（検証ケース・テスト名・期待値の導出根拠・自動化しない範囲）。必要なフィクスチャもここで決める。
3. Red: テスト設計どおりのテストを書き、CI/ローカルで失敗を確認し、失敗ログ（コマンドと出力）を残す。
4. 最小実装でテストを通す。
5. リファクタ → 追加テストの順で進め、検証結果と残課題をまとめて提出。

## 品質基準
- テストが具体的な成功/失敗条件を持ち、境界値と異常系を含む。
- 初回失敗ログ（コマンドと出力）が提出物にある。
