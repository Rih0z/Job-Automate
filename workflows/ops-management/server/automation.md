# WordPress管理システム構築・運用ワークフロー（例）

利用時は認証情報を利用する環境に更新し、このファイルの名前をClaude.mdに書き換えてサーバーのルートに配置してください。

## プロジェクト概要

対象ブログはレンタルサーバー（例: ConoHa）で運用されているWordPressサイトです（ジャンル例: クラフトビール専門ブログ）。Claudeを使ってローカル環境で記事の管理・編集・ファクトチェックを行うシステムを構築します。

## ユーザーの意図とプロジェクト目標

### 1. 基本的な管理システム構築
- **目的**: 対象ブログをClaudeでローカル管理できるようにする
- **要件**: ローカル環境で編集 → 問題なければサーバーにアップロード
- **実現**: SSH接続によるファイル同期システム、バックアップシステム構築

### 2. 記事のバックアップとバージョン管理
- **目的**: 全ての記事をローカルにバックアップし、GitHubで管理
...
### データ管理
- **記事バックアップ**: `blog-posts-backup/`
- **処理済み記事**: `blog-posts-processed/`（ガイドライン適用済み）
- **検証済み記事**: `blog-posts-verified/`（ファクトチェック済み）
- **GitHubリポジトリ**: 非公開リポジトリに保存

## 現在の課題と次のステップ

### 現在の状況
- 記事の処理とファクトチェックは完了
- ただし、本番環境への更新は中断中（対象記事数が膨大な場合は処理時間の問題が生じうる）
...
### 推奨する次のアプローチ
1. **対象記事の絞り込み**: 全記事ではなく、対象ジャンルに合致する記事に限定
...
### プロンプトファイルの配置場所
- **ディレクトリ**: 運用対象サーバー/リポジトリの `server/prompt/` ディレクトリ相当の場所（ローカル作業ディレクトリ配下）
...
2. **国内業界情報収集プロンプト**
   - **ファイル名**: `domestic-industry-research.md`
   - **用途**: 対象ジャンルの国内業界情報収集

3. **海外業界情報収集プロンプト**
   - **ファイル名**: `global-industry-research.md`
   - **用途**: 対象ジャンルの海外業界情報収集

4. **WordPress記事HTML編集マスターガイド**
   - **ファイル名**: `wordpress-html-editing-guide.md`
   - **用途**: 記事の編集・更新作業

5. **記事アフィリエイト商品挿入プロンプト**
...
1. **WordPress記事HTML編集マスターガイド** (`wordpress-html-editing-guide.md`)
   - キャラクターシステムの導入
   - ファクトチェック済みバッジ
   - SEO最適化要素
   - 免責事項の追加

2. **記事ファクトチェックガイド** (`fact-checking-guide.md`)
   - 確認済み事実セクションの追加
   - 主観的評価の明示
   - 情報源の明記

3. **記事アフィリエイト商品挿入プロンプト** (`affiliate-product-insertion.md`)
   - 購入リンク
   - 関連グッズの紹介
   - ペアリング商品

4. **モバイル最適化レビュー構成要素** (`mobile-optimized-review-structure.md`)
   - レスポンシブデザイン
   - タッチフレンドリーUI
   - 表示速度最適化
...
**キャラクター設計の例**:
- サイトの世界観に応じてキャラクターを複数体設定し、役割ごとに語り口・語尾を分離する
  （例: 技術的・客観的情報担当／主観的な感想担当／提案・楽しみ方担当）
1. **更新対象記事の選定**
...
3. **AIによる内容理解と改善案作成**
   ...
   - 文字化け箇所を文脈から推測して修正
   - **以下の更新プロンプトを適用**：
     - `wordpress-html-editing-guide.md` - 基本構造とキャラクターシステム
...
- **複数キャラクターシステム**: 役割ごとに語り口・語尾を分離（技術的情報担当／主観的感想担当／提案・楽しみ方担当 等）
...
- **対策**:
  - **正確な画像マッピング**をプロンプトに明記する（キャラクター名とアイコンID/ファイル名の対応表を維持）
