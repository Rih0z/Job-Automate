...
3. セクションが参照する skills（`review-gate`・`single-session-tdd`・`stop-ai-slop-jp` 等）は各 SKILL.md の導入ノートに従い対象の `.claude/skills/` にコピーし、観点定義（criteria JSON 等）を対象プロジェクト固有に調整する。
4. 対象の CLAUDE.md 本体の生成・剪定は `agent-harness-bootstrap` skill の手順で行う。
5. 移植完了後、**別エージェント**に「本 CLAUDE.md の各セクション vs 対象リポジトリの反映状況」の突合レビューをさせ、抜けゼロを確認してから完了報告する。
