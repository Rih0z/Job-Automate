...
作業中に本題と別件の問題・改善点（バグ・逸脱・想定外の挙動を含む）に気づいた場合、上記「AIが作業中に別の問題を見つけても、その場で勝手に修正させない」原則（[README.md「ハーネスを組むループ」](README.md#ハーネスを組むループ--作成プロンプトも人間は書かない)）どおり、その場で修正に着手せず、まず「何を見つけたか」の事実だけをその応答内で一言報告する。それに加えて、気づいた**その瞬間**に `.tmp/discovery-log.md` へ1行（気づいた内容・手がかりとなるファイル/行・日時）も追記し、口頭報告だけでは長い作業の中で読み手・書き手双方の記憶から埋もれてしまう事態を防ぐ。この記録は正式なタスク化・修正の代わりではなく、人間が直すかどうか・いつ直すかを判断するまでの一時的な備忘バッファであり、イテレーション完了時（または `/compact` 実行前）に必ず見返して、対応済みにするか、新しい workflow/Issue として切り出すかを決めてから当該行を消す（残したまま「完了」と報告しない）。
