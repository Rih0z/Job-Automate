...
## 他プロジェクトのセットアップ依頼への対応 — 完全移植ルール (2026-09-01)
...
このリポジトリのノウハウを使って別のディレクトリ/リポジトリをセットアップするよう依頼された場合、CLAUDE.md の内容を**部分的に**持っていくことを禁止する。以下の手順で全内容を移す:

1. 本 CLAUDE.md の全セクションを列挙した**移植チェックリスト**を最初に作る。
2. 各セクションを「移植する（対象の規模・技術に合わせて適応）」か「N/A（理由を明記）」のどちらかに必ず倒す。黙って省略しない。判断一覧はユーザーに提示する。
3. セクションが参照する skills（`review-gate`・`single-session-tdd`・`stop-ai-slop-jp`・`repo-hygiene-patrol` 等）は各 SKILL.md の導入ノートに従い対象の `.claude/skills/` にコピーし、観点定義（criteria JSON 等）を対象プロジェクト固有に調整する。**`test-verify.md` の「数値目標の単一 SoT 化」条文は必須項目**（N/A 判定不可。対象プロジェクトが実コードのカバレッジ%を持たない場合は「合格率」「品質スコア」等の類似閾値に読み替えて適用する）であり、対応する検査手段として `repo-hygiene-patrol` の「数値目標の整合性ドリフト」チェックも併せてコピーする。
4. 対象の CLAUDE.md 本体の生成・剪定は `agent-harness-bootstrap` skill の手順で行う。
5. 移植完了後、**別エージェント**に「本 CLAUDE.md の各セクション vs 対象リポジトリの反映状況」の突合レビューをさせ、抜けゼロを確認してから完了報告する。**必須項目（N/A 判定不可と明記されたセクション、例: 上記3の数値目標単一 SoT 化条文）が誤って N/A 判定・省略されていないかも突合対象に含める**。

背景: セットアップ時に一部の観点だけ移植され、後から「あの観点は取り込めているか」という確認・追加依頼が繰り返される失敗パターンへの対策。移植の完全性は依頼者の記憶でなくチェックリストと突合レビューで担保する。
