...
- **具体化の粒度(2026-09-03 追加)**: 「ドメイン固有のセキュリティ要件」のような抽象語のままcriteriaに残さず、実在する固有名詞に置き換える。例: `security` criterionの checks を「秘密情報がログに出ない」で止めず「〈そのプロジェクトが実際に扱う認証方式の名称〉のトークンが〜」のように、そのプロジェクトのREADME/設定ファイルから拾える具体名詞(認証方式・主要な外部サービス名・その業界特有の規制用語等)まで踏み込んで書く。抽象語のままのcriterionはレビュアーが一般論で採点してしまい、そのプロジェクト固有の失敗パターンを拾えない。design/implementationのcriteriaに「そのプロジェクトの設計テーマ(ドキュメントに明記された言葉)に反していないか」を1criterionとして持たせるのも同じ理由。
