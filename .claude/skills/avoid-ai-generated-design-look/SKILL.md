...
- **絵文字アイコン多用**: UIに絵文字を頼りすぎ（**全面禁止 — `ui-design-guidelines` Skill参照**）
...
```css
:root {
  /* Brand Identity — customer-persona-design Skill Phase 4-1 の結果を設定 */
...
原文: `workflows/software-development/design/design-research.md`。関連: `ui-design-guidelines`（絵文字禁止・アクセシビリティ実装チェックリスト）、`ibm-carbon-design-system`（デザイントークン本体）、`customer-persona-design`（ブランドトークンの導出元）。
