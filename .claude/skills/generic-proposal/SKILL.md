...
description: "食品・飲料・サプリ・化粧品・アパレル・家電・サービス業など業界を問わず使える汎用ビジネス企画書テンプレート。企画タイトル・分野・組織名・ターゲット市場・差別化要素などの入力から、市場背景・製品比較・価値提案・ビジネスインパクト・競合分析（モート/カウンター戦略）・エンゲージメント設計・生産展開計画・リスク分析までを含む完全な企画書（MD形式）を生成する。「汎用の企画書を作って」「新商品の企画書を書きたい」「業界問わず使える提案書テンプレートで」等で使用。"
---
...
10. **エンゲージメント設計（オプション）**: [ユーザーが使い続けたくなる仕組みのアイデア：習慣ループ・報酬設計・ネットワーク効果等]
...
### 7.7 競合の強みに対するカウンター戦略

[主要競合の最大の強みを特定し、それを無力化または上回るための具体的な戦略。]

| 競合 | 競合の強み | カウンター戦略 | 実現時期 |
|------|-----------|--------------|----------|
| [競合A] | [強み] | [戦略] | [時期] |
| [競合B] | [強み] | [戦略] | [時期] |
...
[時間の経過とともに競争優位がどのように強化されるかの分析。]

- **ネットワーク効果によるモート**: [ユーザー増加→価値向上→さらにユーザー増加のフライホイール]
- **データ蓄積によるモート**: [利用データの蓄積→AI/レコメンド精度向上→体験価値向上]
- **ブランド・信頼によるモート**: [顧客との信頼関係構築→口コミ→新規獲得コスト低下]
- **スイッチングコストによるモート**: [蓄積データ・設定・人間関係→乗り換えの心理的障壁]

### 7.9 競合の次の一手と先回り戦略

[主要競合が今後取りうる戦略的動きの予測と、それに対する準備。]

## 8. エンゲージメント・リテンション設計

### 8.1 習慣ループと行動設計

[ユーザーが製品/サービスを継続的に利用するための行動設計。Nir Eyal の Hooked Model に基づく4要素の設計。]

| 要素 | 設計内容 | 具体例 |
|------|----------|--------|
| **トリガー（きっかけ）** | [外的/内的トリガーの設計] | [具体的な実装方法] |
| **アクション（行動）** | [最小限の摩擦で実行できる行動] | [具体的なUX設計] |
| **変動報酬** | [予測不可能な報酬による期待感] | [達成報酬・社会的報酬・自己報酬の例] |
| **投資** | [ユーザーがサービスに蓄積する価値] | [データ・設定・人間関係・実績] |

### 8.2 行動経済学・心理学の活用

[ユーザーの意思決定とエンゲージメントを促進する行動経済学の原理の活用。]

- **損失回避**: [失うことへの恐れを活用したエンゲージメント施策]
- **社会的証明**: [他ユーザーの行動・レビュー・利用状況の可視化]
- **進捗効果（エンダウド・プログレス効果）**: [進捗バー・達成度・成長の可視化]
- **ピーク・エンドの法則**: [体験のピークモーメントと最終印象の最適化]
- **所有効果**: [カスタマイズ・パーソナライゼーションによる愛着形成]

### 8.3 ネットワーク効果とスイッチングコスト

[ユーザー増加に伴い製品/サービスの価値が向上する仕組みと、競合への乗り換えを抑制する要素の設計。]

- **直接的ネットワーク効果**: [ユーザー同士の接続・コミュニケーションによる価値向上]
- **間接的ネットワーク効果**: [補完サービス・コンテンツ・パートナーの充実]
- **データロックイン**: [使い続けるほど精度が上がるパーソナライゼーション・レコメンド]
- **コミュニティ形成**: [帰属意識・人間関係による離脱コスト]
- **エコシステム統合**: [他サービス・ツールとの連携による離脱障壁]

### 8.4 ゲーミフィケーション要素

[製品/サービスにゲーム的な要素を組み込み、エンゲージメントを高める設計。]
...
- 元プロンプト（詳細確認用）: `workflows/business-planning/generic-proposal.md`
