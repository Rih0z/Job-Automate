...
| 群 | 該当 | 提示の仕方 |
|---|---|---|
| A | `provenance: official` | 「デフォルト採用。外すものがあれば申告」 |
...
- **対話が可能な時**: `AskUserQuestion` を使う。ツールの制約は **1 回の呼び出しで最大 4 問・各問の選択肢は 2〜4 個・header は 12 文字以内**なので、群 C は「4 択 × 最大 4 問 = 1 呼び出し」を単位に分割し、**先に問数と呼び出し回数の見込みを 1 行で宣言**してから始める（例: 「著者嗜好 30 要素 → 8 問・2 呼び出し」）。選択肢の label は要素 id、description は台帳の `summary` を使う。`multiSelect: true`。群 A・B は最後に「外す要素はあるか」を 1 問で聞く。
...
別エージェントに渡すのは **対象パス 3 点と観点定義のパスの計 4 点のみ**（会話履歴・実装意図・本スキルの他の部分は渡さない）:

1. このリポジトリの `provenance.json` の完全パス
2. 対象の `.claude/harness-selection.json` の完全パス
3. 対象プロジェクトのルートの完全パス
4. 観点定義 `criteria/porting-reconciliation.json` の完全パス（review-gate 形式。記録の完全性 / 抜けゼロ / 混入ゼロ / 依存整合 / 報告の開示）

レビュアは `review-gate` skill と同じ手順で criteria を検証し JSON で結果を返す（must_pass 全合格かつ min_score 以上で PASS）。FAIL なら findings を修正して再依頼する。3 回 FAIL で中断してユーザーに報告する（対象で `issue-lifecycle` が選択済みなら起票、非選択なら報告のみ）。
