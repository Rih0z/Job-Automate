...
- **独自 skill を新規作成する場合**: `skill-authoring-guide` の「決定的な強制 vs 確率的な依頼」（本当に保証したい規律は指示文でなく検証スクリプト/スキーマで強制する）・「Few-shot 例の設計」（2〜4個・多様性重視・欠損値の例を1つ含める）・「個人用にカスタマイズする場合」（`~/.claude/skills/` へ別名コピー、shadowing 回避）の 3 観点を、生成した skill のレビュー基準に含める
- **独自 MCP ツールを実装する場合**: `mcp-server-setup` の「カスタムツール実装時のエラー応答設計」（エラー種別の区別・「本当に失敗」と「正常処理だが結果が空」の区別・リトライ可否の明示）を、生成したツール実装のレビュー基準に含める

いずれも Step 1 事実収集で「独自 skill / MCP ツールを新規作成するか」を確認し、該当する場合のみ Step 7 のレビュー観点に追加する。
...
> provenance: author-preference · id: changelog-practice

harness（CLAUDE.md / rules / skills）への変更を、日付付きの短い1行エントリとして `CHANGELOG.md`（対象リポジトリルート）に追記する運用。本文中に「(2026-09-XX 追加)」のような日付注記を散在させる代わりに、変更履歴を1箇所に集約する。**Anthropic 公式ベストプラクティスに直接の根拠はない**、著者の運用嗜好（他の多くの要素と異なり `depends_on` は無し・単独で採否を決められる）。既存の `CHANGELOG.md` がある対象では新規作成せず追記運用に合流する。`docs-management`（`docs/` 配下の運用）とは独立しており、`docs/` の有無に関わらずルートに置く。
