# 技術文書モード(日本語技術文書の文章規範)

日本語で技術的な原稿(書籍の章、記事、解説文)を書く・推敲するときの規範。ブログ・体験記向けのコアルールとは前提が違うので、このモードでは一部のコアルールを上書きする(上書き表は `SKILL.md` の「技術文書モード」を参照)。

`phrases.md` / `structures.md` の語彙・構造カタログはこのモードでも有効。このファイルは、それらに無い「論証・段落・読者負荷」の規範を足す。
...
- ダッシュ(em ダッシュ —、horizontal bar ―、2倍ダッシュ「——」)を日本語の地の文・見出しで使わない。同格・補足の挿入(「A——挿入——B」)は括弧()に、言い換え・敷衍(「A——B」)は句点で二文に分けるか読点でつなぐ。範囲を示す en ダッシュ – や英語の複合語(Curry–Howard など)、コードブロック・書誌情報は対象外。
...
- 「AではなくB」と否定するときは、否定の根拠を一文添える。反実仮想(「もしAなら、〜だっただろう」)が使えることが多い。(二項対比の多用自体は `structures.md` 2-1 のとおり避ける。)
...
- 例示では、結果の羅列や受動態(「特定され、判明した」)ではなく、行為者を主語にした動作の連なり(「リポジトリを調査して特定し、見つけてくれた」)で書く。(false agency の技術文書版。`structures.md` 系統1と同根。)
...
- 短い決め台詞を独立した段落にして緊張を作る演出を多用しない。段落内の短い体言止め(「ここまでわずか数十秒。」など)は、場面の山場に限り使ってよい。(`structures.md` 2-3「劇的な断片化」の技術文書版。全面禁止ではなく山場限定。)
...
LLM が大量生成する、中身のない型に誘惑されない。書き上げたら、この節で点検する。書いている本(文書)の術語を議論に使うのはよい。空虚な装飾として使うのが問題である。

次のような言い回しは、論点を増やさず「ちゃんと書いている感」だけを付ける LLM 口調である。使わない。(`phrases.md` 4・5 系統と重なる。以下は技術文書で特に出やすいもの。)
...
- 見出しを、節の結論を言い切る「セリフ」にしない。見出しの時点で読者がオチを知る状態を避ける。(`structures.md` 3-1「命題型H2」と同じ帰結。)
- 節で扱っている対象を指す名詞句でもよい。
