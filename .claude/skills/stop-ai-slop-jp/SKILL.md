...
2026-08-31 追加。文体の良し悪し（slop採点）ではなく**事実の誤り**であり、スコアに関わらず最優先で是正する。チェック項目本体は `criteria/scoring.json` の `blocker_checks`（帰属誤り・属性の過小断定）を SoT とする。
...
観点定義(SoT) = `.claude/skills/stop-ai-slop-jp/criteria/scoring.json`(5軸の質問文・1/5/10点アンカー・合格閾値・BLOCKERチェックを保持、本文へ prose 重複記載しない)。5軸を1〜10で採点し合計35/50を下回ったら書き直す(中間点は両隣アンカーの間で判断)。`blocker_checks`(帰属誤り・属性の過小断定)はスコア対象外・事実の誤りとしてスコアに関わらず最優先で是正する。
